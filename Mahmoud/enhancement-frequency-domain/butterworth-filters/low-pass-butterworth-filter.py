...
axs[0, 1].imshow(butterworth_lowpass_filter(img, 5, 2), cmap='gray')
axs[0, 1].set_title('D0 = 5')

axs[1, 0].imshow(butterworth_lowpass_filter(img, 15, 2), cmap='gray')
axs[1, 0].set_title('D0 = 15')

axs[1, 1].imshow(butterworth_lowpass_filter(img, 30, 2), cmap='gray')
axs[1, 1].set_title('D0 = 30')

fig.suptitle('Applying Butterworth Lowpass Filter n = 2')
# display the plot
plt.show()

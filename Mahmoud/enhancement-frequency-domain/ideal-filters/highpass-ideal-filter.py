...
def highpass_ideal_filter(img, D0):
  fft = np.fft.fft2(img)
...
axs[0, 1].imshow(highpass_ideal_filter(img, 15), cmap='gray')
axs[0, 1].set_title('D0 = 15')

axs[1, 0].imshow(highpass_ideal_filter(img, 30), cmap='gray')
axs[1, 0].set_title('D0 = 30')

axs[1, 1].imshow(highpass_ideal_filter(img, 80), cmap='gray')
axs[1, 1].set_title('D0 = 80')
fig.suptitle('Applying Ideal HighPass Filter')
# display the plot
plt.show()

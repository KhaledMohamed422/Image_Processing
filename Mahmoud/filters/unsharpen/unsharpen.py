...
# for faster smoothing i am using the the built-in 
## but when i write the smoothing i write both for assighnment
...
def apply_unsharpen(img):
    smooth = apply_gaussian_filter_builtin(img, 2).astype('int16')
    edge = np.clip(img - smooth, 0, 255).astype('int16')
    return np.clip(edge + img, 0, 255).astype(np.uint8)

org = cv.imread('org.png')
# cv.imshow('original', org)
# cv.waitKey(0)

out = apply_unsharpen(org)
...
axs[1].set_title('unsharpen Image')
plt.show()